...
function AddTrainer() {
  ...
  const validateForm = () => {
    ...
    if (!formData.branch_ids || formData.branch_ids.length === 0)
      newErrors.branch_ids = "At least one branch must be selected.";
    setErrors(newErrors);
  ...
  const handleEdit = async () => {
    try {
      ...
      if (response.data.valid) {
        ...
        // tempData.branch_codes=tempData.branch_codes.map((branch) => ({  }));
  ...
  const handlePdfUpload = async (files) => {
    ...
    try {
      const result = await userService.uploadImage(
        '/v0/image/upload?folderName=connectx/trainer',
        data
      );
                ...
                <Col xs={4} className="mb-2 mt-4">
                  ...
                  <Autocomplete
                    ...
                    onChange={(e, value) => {
                      setFormData((prev) => ({
                        ...prev,
                        branch_ids: value.map((val) => val.branch_id),
                        branch_codes: value,
                      }));
