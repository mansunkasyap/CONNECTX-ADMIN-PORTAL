...
export default function CustomerCheckPoints() {
			...
			},
			{
				...
				Cell: ({ row }) => (
					<RowActionMenu
						row={row.original}
						onEdit={handleEdit}
						onPrint={(data) => handlePrint(data.checkpoint_id)}
	...
	const handleEdit = (data) => {
		const obj = { ...data };
		obj.p_action = "update";
		navigate("/customers/customer-check-points/add", {
			state: {
	...
	const handlePrint = (data) => {
		const url = `${config.reactUrl}customers/checkpoint-print/${data}`;
		window.open(url, "_blank");
	};

	// const handlePrint = (data) => {
	//   window.open(`${config.reactUrl}/customers/customer-check-points/add/${data}`);
	// };
	...
	return (
		<MainLayout
			...
			linkto={"/customers/customer-check-points/add"}
